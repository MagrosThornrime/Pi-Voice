...
void midiHandler(const midi::Data data) noexcept {
    ...
    if (data.status() == midi::Data::noteOn) {
        g_voiceManager->turnOn(data.note().num);
    } else if (data.status() == midi::Data::noteOff) {
        g_voiceManager->turnOff(data.note().num);
    }
}
...
// Open selected MIDI port (and start callback)
void openMidi(const Napi::CallbackInfo& info) {
    auto env = info.Env();
    if (info.Length() != 1) {
        Napi::Error::New(env, "openMidi(portNum)").ThrowAsJavaScriptException();
        return;
    }
...
void startSynth(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        pipeline
            .setSource(g_voiceManager)
            .addLayer(std::make_shared<LowPassFilter>(2, 2, 1000, 44100))
            .addLayer(g_recorder);

        g_stream = std::make_unique<portaudio::InterfaceCallbackStream>(
            streamParams,
            pipeline
        );
